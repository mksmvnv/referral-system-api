...
async def referrer_register(
    ...
    if not register.referral_code:
        ...
        await referral_service().register(redis=redis, register=register)
...
async def create_referral_code(
    ...
    referral_code = await referrer_service().create_referral_code(
        redis=redis, current_user=current_user
    )
...
async def delete_referral_code(
    ...
    return DeletedReferralCodeResponse(status="not deleted")
